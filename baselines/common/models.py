...
def gym_microrts_cnn(unscaled_images, **conv_kwargs):
    """
    CNN from Nature paper.
    """
    scaled_images = tf.cast(unscaled_images, tf.float32)
    activ = tf.nn.relu
    h = activ(conv(scaled_images, 'c1', nf=16, rf=3, stride=2, init_scale=np.sqrt(2),
                   **conv_kwargs))
    h2 = activ(conv(h, 'c2', nf=32, rf=2, stride=1, init_scale=np.sqrt(2), **conv_kwargs))
    h3 = conv_to_fc(h2)
...
@register("cnn_gym_microrts")
def cnn_gym_microrts(**conv_kwargs):
    def network_fn(X):
        return gym_microrts_cnn(X, **conv_kwargs)
    return network_fn
